...
[bLASSOv, FitInfo] = lasso(X, EDduration, "CV", 10, "Lambda", lambda);
idxMinMSE = FitInfo.IndexMinMSE;
b0 = FitInfo.Intercept(idxMinMSE);
bLASSO = bLASSOv(:, idxMinMSE);
mask3 = (bLASSO ~= 0);
...
% We conclude that the model without the Spike Variable is
% better than the one with it. The variance of the MSE is higher and that
% is explained by the fact that we don't delete any values like before, at
% the same time the adjR2 metric is a lot higher which suggests way better